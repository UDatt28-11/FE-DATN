...
const BookingInfoPage: React.FC = () => {
  ...
  const normalizeBookingData = (): BookingData => {
    ...
    if (locationState.rooms && locationState.rooms.length > 0) {
      ...
      const newRooms = locationState.rooms.filter(
        (newRoom) =>
          !existingRooms.some(
    ...
    try {
      ...
      const details = rooms.map((room) => {
        ...
        if (room.checkIn && room.checkOut) {
          ...
          checkOutDate = dayjs(room.checkOut, "DD/MM/YYYY").format(
            "YYYY-MM-DD"
          );
  ...
  return (
    <div className="booking-info-page">
      ...
      <Content
        style={{ padding: "40px 0", minHeight: "70vh", background: "#fff" }}
      >
        <div className="container">
          {/* Nút Trở về */}
          <Button
                  ...
                  <Row gutter={16}>
                    ...
                    <Col xs={24} sm={12}>
                      ...
                      >
                        <Input
                          prefix={
                            <PhoneOutlined style={{ color: "#cb8670" }} />
                          }
                    ...
                    {/* Địa chỉ đã bị loại bỏ theo yêu cầu */}
                  </Row>
