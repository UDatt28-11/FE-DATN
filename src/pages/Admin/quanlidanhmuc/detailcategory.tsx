...
const DetailCategory: React.FC<DetailCategoryProps> = ({
  ...
  const [roomTypeWithServices, setRoomTypeWithServices] =
  ...
  const [selectedRoomForSupplies, setSelectedRoomForSupplies] =
  ...
  const loadRoomImages = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/admin/room-types/${roomType.id}`
      );
      ...
      if (response.data.success && response.data.data) {
        ...
        if (roomTypeData.images && Array.isArray(roomTypeData.images)) {
          allImages.push(
            ...roomTypeData.images.map((img: any) => ({
              id: img.id,
        ...
        if (
          roomTypeData.image_url &&
          !allImages.some((img) => img.image_url === roomTypeData.image_url)
        ) {
          allImages.unshift({
            id: 0,
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message ||
          "Không thể tải danh sách vật tư của phòng"
      );
      setRoomSupplies([]);
    } finally {
  ...
  const handleEditRoom = async (room: Room) => {
    try {
      ...
      const response = await roomService.getRoomById(
        room.id,
        "property,roomType,roomType.images,amenities"
      );
      ...
      <Row gutter={24}>
        <Col span={10}>
          <Image
            ...
            preview={{
              src:
                displayRoomType.image_url || "https://via.placeholder.com/300",
            }}
          />
        </Col>
        <Col span={14}>
          ...
          <Paragraph style={{ marginTop: 16 }}>
            {displayRoomType.description || "Chưa có mô tả"}
          </Paragraph>
          <Descriptions
            column={1}
            bordered
          ...
          >
            ...
            <Descriptions.Item label="Sức chứa chuẩn">
              {displayRoomType.max_adults ?? 0} người lớn,{" "}
              {displayRoomType.max_children ?? 0} trẻ em
            </Descriptions.Item>
            <Descriptions.Item label="Property">
              {displayRoomType.property?.name || "-"}
            </Descriptions.Item>
            <Descriptions.Item label="Dịch vụ">
              {displayRoomType.services &&
              displayRoomType.services.length > 0 ? (
                <Space size={[4, 4]} wrap>
        ...
        >
          ...
          <Upload
            ...
            beforeUpload={async (file) => {
              ...
              try {
                const res = await roomtypeService.uploadImages(
                  roomType.id,
                  formData
                );
                if (res.success) {
                ...
                toast.error(
                  error.response?.data?.message || "Có lỗi khi tải ảnh"
                );
              } finally {
      ...
      >
        ...
        >
          <div>
            ...
            <br />
            <span>Tầng: {selectedRoomForSupplies?.floor_number ?? "-"}</span>
          </div>
          <Space>
