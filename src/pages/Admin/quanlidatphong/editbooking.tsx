...
import { getBooking, updateBooking } from "../../../service/bookingService";
import type {
  BookingOrder,
  UpdateBookingData,
...
const EditBooking: React.FC = () => {
  ...
  const fetchBookingDetail = async (bookingId: number) => {
    try {
      ...
      form.setFieldsValue({
        ...
        payment_method:
          paymentMethodDisplayMap[data.payment_method || "cash"] || "Tiền mặt",
        notes: data.notes || "",
      });
    } catch (error: any) {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message ||
          "Không thể cập nhật đặt phòng. Vui lòng thử lại!"
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {/* Card form chỉnh sửa */}
      <Card title={`✏️ Sửa đặt phòng #${booking.code}`} variant="borderless">
        <Form form={form} layout="vertical" onFinish={handleEditBooking}>
          ...
          >
            <p>
              <strong>Mã đơn:</strong> {booking.code}
            </p>
            <p>
            ...
            <p style={{ margin: 0 }}>
              <strong>Số phòng đặt:</strong> {booking.details_count} phòng
            </p>
            <p style={{ margin: "8px 0 0 0", fontSize: 12, color: "#8c8c8c" }}>
              💡{" "}
              <em>
                Để sửa thông tin phòng, ngày check-in/out, vui lòng liên hệ quản
                trị viên
              </em>
            </p>
          </div>
