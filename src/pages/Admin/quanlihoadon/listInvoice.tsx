...
const ListInvoice: React.FC = () => {
  ...
  // Apply filters
  const applyFilters = (search: string, status: string, paymentStatus: string) => {
  ...
  const handleStatusChange = async (id: number, newStatus: Invoice["invoice_status"]) => {
    try {
  ...
  const getInvoiceStatusTag = (status: Invoice["invoice_status"]) => {
    const statusConfig = {
      ...
      sent: { color: "processing", icon: <ClockCircleOutlined />, text: "Đã gửi" },
  ...
  const columns: ColumnsType<Invoice> = [
    {
      ...
      width: 130,
      render: (text) => (
        <span style={{ fontWeight: 600, color: "#1890ff" }}>{text || "N/A"}</span>
      ),
    },
    {
      title: "Khách hàng",
    ...
    },
    {
      ...
      align: "right",
      render: (balance) => (
        <span style={{ fontWeight: 500, color: (balance || 0) > 0 ? "#ff4d4f" : "#52c41a" }}>
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          {record.payment_status !== "paid" && record.invoice_status !== "cancelled" && (
          ...
          {record.invoice_status !== "cancelled" && record.invoice_status !== "paid" && (
            <Popconfirm
