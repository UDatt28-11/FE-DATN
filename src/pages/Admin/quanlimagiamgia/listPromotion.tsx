...
const ListPromotion: React.FC = () => {
  ...
  const handleBulkDelete = async () => {
    try {
      await Promise.all(
        selectedRowKeys.map((id) => promotionService.remove(id as number))
      );
      ...
      toast.success(
        `Đã xóa ${selectedRowKeys.length} mã giảm giá khỏi hệ thống.`
      );
      ...
      toast.error(
        error.response?.data?.message ||
          "Không thể xóa các mã giảm giá. Vui lòng thử lại."
      );
    }
  };
  ...
  const columns: ColumnsType<Promotion> = [
        ...
        <div>
          ...
          {record.max_discount_amount && (
            <div style={{ fontSize: 12 }}>
              <PercentageOutlined /> Tối đa:{" "}
              {record.max_discount_amount.toLocaleString("vi-VN")}₫
            </div>
          )}
        </div>
      ),
    },
    {
      ...
      render: (_, record) => (
        <div>
          <div>
            Đã dùng: {record.usage_count || 0} / {record.max_usage_limit || 0}
          </div>
    ...
    },
    {
      ...
      render: (_, record) => (
        <div style={{ fontSize: 12 }}>
          <div>
            Bắt đầu: {new Date(record.start_date).toLocaleDateString("vi-VN")}
          </div>
          <div>
            Kết thúc: {new Date(record.end_date).toLocaleDateString("vi-VN")}
          </div>
        </div>
      ),
    },
